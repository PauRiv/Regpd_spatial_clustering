...
source("/scratch3/pauline/Regpd_spatial_clustering/Regionalisation_fit/parallelized_regional_inference_functions.R")
...
for(clust_nb in 1:nclust){
  ...
  List_reg_fit[[clust_nb]] <- fitEGPDkSemiRegCensoredIter_paral(M = t(M_pos_precip), ncores = floor(detectCores()/3 - 1))
  
}#end for clust_nb

save(List_reg_fit, file = paste0("/scratch3/pauline/Regpd_spatial_clustering/Regionalisation_fit/result_reg_fit_",seas,"V1.Rdata"))
save(List_keep_coord, file = paste0("/scratch3/pauline/Regpd_spatial_clustering/Regionalisation_fit/keep_coord_reg_fit_",seas,"V1.Rdata"))