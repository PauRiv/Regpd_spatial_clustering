...
# DON'T USE THOSE
fitEGPDkSemiRegCensoredIter.boot <- function(M,method="pwm",cens_thres=c(0,Inf),round=0.1,
  ...
  #at-site parameters for each site
  for (station in 1:nstat) {
    y =na.omit(M[,station])
    fit_init = fit.extgp(y,model=1,method=method,init =ParInit, censoring = cens_thres, rounded = round,plots = FALSE)
    kappa_init[station] = fit_init$fit$pwm[1]
...
fitEGPDkREGCensorIter.boot <- function(M,method="pwm",cens_thres=c(1,Inf),round=0.1,
                                  sites="default", thres=0, precision=.0001,
  ...
  #estimate at site parameters for each site
  for (station in 1:nstat) {
    y = na.omit(M[,station])
    fit_init = fit.extgp(y,model=1,method=method,init =ParInit, censoring = cens_thres, rounded = round,plots = FALSE)
    kappasite_init[station] = fit_init$fit$pwm[1]
  ...
  Theta_loc=list("kappai" = kappasite_init,"sigmai"=sigma_init,"xii"=xisite_init)
  #init of regional parameters for sites of interest
  kappa_init = mean(kappasite_init)
  xi_init = mean(xisite_init)
  Theta_init = list("kappa.reg"=rep(kappa_init,nb_stat),"sigma"=sigma_init[sites],"xi.reg"=rep(xi_init,nb_stat))
  
  Theta = Theta_init
  Theta_0 = list("kappa.reg"=rep(kappa_init,nb_stat),"sigma"=sigma_init[sites],"xi.reg"=rep(xi_init,nb_stat),
                 "kappa.site"=kappasite_init[sites],"xi.site"=xisite_init[sites])
  loop <-0; increment <-precision +.1 
  log.init<-0
  Xi.old <- unique(Theta$xi.reg)
  Kappa.old = unique(Theta$kappa.reg)
